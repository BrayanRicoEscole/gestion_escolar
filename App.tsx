...
import { 
  ...
  LogOut, Loader2, FlaskConical, ClipboardList, UserX
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const modules = [
    { id: 'dashboard', name: 'Dashboard', icon: LayoutDashboard, status: 'active' },
    ...
    { id: 'retired_students', name: 'Histórico Retirados', icon: UserX, status: 'active' },
        ...
        <div className="max-w-md w-full bg-white rounded-[3rem] shadow-2xl p-12 text-center border border-slate-100 animate-in fade-in zoom-in duration-500">
          ...
          <div className="space-y-4">
            <button onClick={handleLogin} className="w-full flex items-center justify-center gap-4 py-5 bg-white border-2 border-slate-100 rounded-3xl font-black text-slate-700 hover:bg-slate-50 hover:border-slate-200 transition-all active:scale-95 shadow-sm">
              <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" className="w-6 h-6" /> Continuar con Google
            </button>
            <button onClick={handleBypass} className="w-full flex items-center justify-center gap-2 py-3 bg-slate-50 text-slate-400 rounded-2xl font-bold text-xs hover:bg-slate-100 hover:text-slate-600 transition-all border border-transparent hover:border-slate-200">
              <FlaskConical size={14} /> Acceso de Pruebas (Bypass)
            </button>
          </div>
          <p className="mt-10 text-[10px] text-slate-300 font-black uppercase tracking-widest">© 2026 Red de Colegios Renfort</p>
        </div>
      </div>
    );
  }
  ...
  const renderContent = () => {
    ...
    if (activeModule === 'active_students') return <ActiveStudentsModule />;
    if (activeModule === 'retired_students') return <RetiredStudentsModule />;
    if (activeModule === 'grading') return userRole === 'admin' ? <GradingModule /> : <TeacherGradingView />;
    if (activeModule === 'comments') return userRole === 'admin' ? (
        <div className="flex flex-col items-center justify-center h-full text-slate-500 animate-in fade-in">
           <div className="text-center p-12 bg-white rounded-[3rem] border border-slate-100 shadow-sm max-w-xl">
             ...
             <p className="mt-4 text-slate-500 font-medium">
               Para configurar el catálogo de frases predefinidas (plantillas) que usarán los docentes, diríjase al <strong>Paso 9</strong> de la configuración del año escolar.
             </p>
             <button onClick={() => setActiveModule('grading')} className="mt-8 px-10 py-4 bg-slate-900 text-white rounded-2xl font-black shadow-xl hover:bg-slate-800 transition-all active:scale-95">
               Abrir Configuración
             </button>
           </div>
        </div>
